...
function isDrizzleTable(obj: any): obj is DrizzleTable {
  ...
  // For debugging - log a few examples
  console.log("Checking object:", Object.keys(obj));

  // Check for table properties in different possible Drizzle formats
  return (
    // More thorough checks
    obj.$type === "table" ||
    (obj._ && obj._.name) ||
    obj.name ||
    // For pgTable objects in newer Drizzle versions
    (typeof obj === "object" && "columns" in obj) ||
    // Detect standard Drizzle table structure
    (obj &&
      typeof obj === "object" &&
      Object.values(obj).some(
        (val) =>
          val && typeof val === "object" && "name" in val && "tableName" in val
      ))
  );
}
...
function getTableName(table: any): string {
  ...
  if (Object.values(table).length > 0) {
    const firstCol = Object.values(table)[0];
    if (firstCol && typeof firstCol === "object" && "tableName" in firstCol) {
      return String((firstCol as { tableName?: unknown }).tableName);
    }
  }

  // Last resort - try to find any column with tableName
  for (const key in table) {
    if (
      table[key] &&
      typeof table[key] === "object" &&
      "tableName" in table[key]
    ) {
      return table[key].tableName;
    }
  }
