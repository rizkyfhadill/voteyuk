...
const generateMockTrendData = (candidates: Array<{ id: string; name: string }>) => {
  const now = new Date();
  ...
  return hours.map((time, timeIndex) => {
    const point: TrendPoint = { timestamp: time.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) };
    
    candidates.forEach(candidate => {
      const finalCount = candidate.voteCount || 0;
      // Earlier times have fewer votes, trending upward
      const factor = Math.min(1, (timeIndex + 1) / hours.length + Math.random() * 0.1);
  ...
  const participationTrend = React.useMemo(() => {
    ...
    return trendData.map(point => {
      const totalVotesAtTime = candidates.reduce((sum, candidate) => 
        sum + (point[candidate.name] as number || 0), 0);
    ...
    <div className="space-y-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Percentage of eligible voters who have cast ballots over time</CardDescription>
        </CardHeader>
        <CardContent>
