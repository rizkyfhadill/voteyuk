...
export default function ElectionTiming({ startTime, endTime }: ElectionTimingProps) {
  const [timeRemaining, setTimeRemaining] = React.useState<string>("");
  const [electionStatus, setElectionStatus] = React.useState<"upcoming" | "active" | "ended">("active");
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        {/* Countdown Timer */}
        <motion.div 
          className="py-3 px-4 rounded-md bg-slate-50 text-center"
          animate={{ scale: timeRemaining.includes("s") ? [1, 1.02, 1] : 1 }}
          transition={{ duration: 0.5, repeat: timeRemaining.includes("s") ? Infinity : 0, repeatDelay: 0.5 }}
        >
          <div className="flex items-center justify-center gap-2 mb-1">
            ...
            <span className="text-sm font-medium text-slate-600">Countdown</span>
          </div>
